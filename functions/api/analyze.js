...
// 프로모션 URL을 서버에서 읽어 Claude API로 내용을 분석/분류합니다.
// (브라우저가 아니라 서버에서 fetch하므로 CORS 제약이 없습니다.)
//
// 필요한 시크릿: ANTHROPIC_API_KEY
//   - 로컬:  .dev.vars 파일에 ANTHROPIC_API_KEY=sk-ant-...
//   - 배포:  wrangler pages secret put ANTHROPIC_API_KEY --project-name promoboard
//            (또는 Cloudflare 대시보드 > Pages > Settings > Environment variables)
// ============================================================
...
const GROUP_IDS = [...Object.keys(CATEGORIES.brand), ...Object.keys(CATEGORIES.local)];
...
async function classify(meta, url, apiKey) {
  const system =
    ...
    "카테고리(id: 세부목록):\n" +
    Object.entries({ ...CATEGORIES.brand, ...CATEGORIES.local })
  ...
  // 안전장치: sub가 group에 없으면 첫 항목으로 보정
  const subs = { ...CATEGORIES.brand, ...CATEGORIES.local }[draft.group] || [];
